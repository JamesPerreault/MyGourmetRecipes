...
import defaults.defaults_en as defaults
...
class PossiblyCaseInsensitiveDictionary (dict):
    ...
    def has_key (self, k):
        ...
        if k in super().keys(): return True
        else:
            for t in self.transformations:
                if hasattr(k,t):
                    #if dict.has_key(self,getattr(k,t)()): return True
                    if getattr(k,t)() in self.keys(): return True
        return False
        ...
        if k in super().keys():
            ...
            for t in self.transformations:
                if hasattr(k,t):
                    nk = getattr(k,t)()
                    if nk in super():
                        return super().__getitem__(self,nk)
        # Raise plain old error
        super().__getitem__(self,k)
...
debug("Number_finder_regexp = {}".format(NUMBER_FINDER_REGEXP ), 1)
